...
from utils.rep_loss import RepLoss
...
class ComputeLoss:
    ...
    def __init__(self, model, autobalance=False):
        ...
        self.device = device
        self.rep_loss = RepLoss(alpha=0.5, beta=0.5, sigma=0.25)
    ...
    def __call__(self, p, targets):  # predictions, targets
        ...
        for i, pi in enumerate(p):  # layer index, layer predictions
            ...
            # 根据batch来计算loss，样本匹配同一个batch的目标框和预测框信息
        ...
        if self.hyp['Reploss']:
            return (lbox + lobj + lcls + lrep) * bs, torch.cat((lbox, lobj, lrep)).detach()
